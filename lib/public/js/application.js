$(function() {
    ...
    function updateInfos(infos) {
        ...
        for(var i=0;i<infos.length;i++) {
            station_hash[infos[i].station_name] = infos[i].text;
        }

        for(var i=0;i<stations.length;i++) {
            var station_name = stations[i];
            if(station_hash[station_name]!=undefined) {
                //遅延情報が存在
                //console.log("#"+station_name+"-modal");
                $("#"+station_name).html(station_name+'<br><img src="/images/mark_warning04.gif" height="20px"/>');
                $("#"+station_name+"-modal").html('<a class="modal_close" href="#"></a>'+station_hash[station_name]);
            }else{
                //遅延情報がない
                $("#"+station_name).text(station_name);
                $("#"+station_name+"-modal").html('<a class="modal_close" href="#"></a>遅延情報がありません');
            }
        }
    }
    ...
    ws.onmessage = function(msg) {
        ...
        $('#date_info span').text(new Date(data.const.date).toTimeString());
    ...
    $("div.subway-map span").each(function(){
        ...
        //copy.css({"color":"red", "z-index":6000});
